...
def load_data(data, prefix='data2/'):
	"""
	...
	if data == 'yelp':
		...
		with open(prefix + 'user_labels.pickle', 'rb') as file:
			labels = pickle.load(file)
			labels.flatten()
		file.close()
		feat_data = sp.load_npz(prefix + 'amz_features_25.npz')
...
def test_pcgnn(test_cases, labels, model, batch_size, thres=0.5):
	...
	# gnn_pred_list.extend(gnn_pred.data.cpu().numpy()[:, 1].tolist())
	# gnn_prob_list.extend(gnn_prob.data.cpu().numpy()[:, 1].tolist())
	# label_list1.extend(label_prob1.data.cpu().numpy()[:, 1].tolist())
	...
	return f1_macro_gnn, f1_binary_1_gnn, f1_binary_0_gnn, auc_gnn, gmean_gnn, gnn_pred_list, label_list1
