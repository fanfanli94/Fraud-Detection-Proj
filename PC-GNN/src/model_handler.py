...
class ModelHandler(object):
	...
	def train(self):
		...
		for epoch in range(args.num_epochs):
			...
			if epoch % args.valid_epochs == 0:
				if args.model == 'SAGE' or args.model == 'GCN':
					...
					print("Valid at epoch {}".format(epoch))
					f1_mac_val, f1_1_val, f1_0_val, auc_val, gmean_val, gnn_pred_list, label_list1 = test_pcgnn(idx_valid, y_valid, gnn_model, args.batch_size, args.thres)
					if auc_val > auc_best:
						f1_mac_best, auc_best, ep_best = f1_mac_val, auc_val, epoch
						if not os.path.exists(dir_saver):
							os.makedirs(dir_saver)
		...
		if args.model == 'SAGE' or args.model == 'GCN':
			...
			f1_mac_test, f1_1_test, f1_0_test, auc_test, gmean_test, gnn_pred_list, gnn_prob_list = test_pcgnn(idx_test, y_test, gnn_model, args.batch_size, args.thres)

			
			# add by me
			performance_log = [auc_test, gmean_test]
			gnn_results = [idx_test, y_test, gnn_pred_list] 
			label_results = [idx_test, y_test, label_list1]
			df1 = pd.DataFrame(np.array(performance_log))
			df1.to_csv("performance_log.csv")
			df2 = pd.DataFrame(np.transpose(np.array(gnn_results)))
			df2.to_csv("gnn_results.csv")
			df3 = pd.DataFrame(np.transpose(np.array(label_results)))
