...
if __name__ == '__main__':
	...
	reviews = getdict('kcore_5.json.gz')

	pickle.dump(reviews, open('kcore_5.pickle', 'wb'))

	with open('kcore_5.pickle', 'rb') as file:
	...
	print("length of unlabeled: "+str(len(unlabel_user)))
	...
	sp.save_npz('amz_uru_adj.npz', user_adj.tocsr())
	with open('amz_uru_adj.pickle', 'wb') as f:
		pickle.dump(user_adj, f)
	f.close()
	exit()
	# construct feature vectors
	features = build_features(new_reviews)
	sp.save_npz('amz_features_36.npz', features.tocsr())

	features = sp.load_npz('amz_features_36.npz')
