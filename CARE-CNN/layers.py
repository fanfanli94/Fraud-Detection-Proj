...
class InterAgg(nn.Module):
	...
	def forward(self, nodes, labels, train_flag=True):
		...
		if self.RL and train_flag:
			...
			df = pd.DataFrame(np.array(self.relation_score_log))
			df.to_csv('relation_log.csv') 
			df = pd.DataFrame(np.array(self.thresholds_log))
			df.to_csv('thresholds_log.csv')
...
def RLModule(scores, scores_log, labels, thresholds, batch_num, step_size):
	...
	if len(scores_log) % batch_num != 0 or len(scores_log) < 2 * batch_num:
		...
		print(f'relation scores: {relation_scores}')
