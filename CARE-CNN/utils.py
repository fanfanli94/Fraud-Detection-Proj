...
def load_data(data):
	...
	if data == 'yelp':
		...
		with open('user_labels.pickle', 'rb') as file:
			labels = pickle.load(file)
			# print(np.where(labels == 1))
			labels.flatten()
		file.close()
		feat_data = sp.load_npz('amz_features_25.npz')
		feat_data = feat_data.todense().A
		# load the preprocessed adj_lists
		# with open(prefix + 'amz_homo_adjlists.pickle', 'rb') as file:
		# 	homo = pickle.load(file)
		with open(prefix + 'amz_uru_adj2.pickle', 'rb') as file:
			homo = pickle.load(file)
		file.close()
		with open(prefix + 'amz_upu_adj2.pickle', 'rb') as file:
			relation1 = pickle.load(file)
		file.close()
		with open(prefix + 'amz_usu_adj2.pickle', 'rb') as file:
			relation2 = pickle.load(file)
		file.close()
		with open(prefix + 'amz_uru_adj2.pickle', 'rb') as file:
			relation3 = pickle.load(file)
...
def test_care(test_cases, labels, model, batch_size):
	...
	for iteration in range(test_batch_num):
		...
		tmp = np.where(np.array(gnn_list) >= 0.5, 1, 0)
		gnn_list = tmp.tolist()
		tmp = np.where(np.array(label_list1) >= 0.5, 1, 0)
		label_list1 = tmp.tolist()
	...
	return auc_gnn, auc_label1, recall_gnn, recall_label1, gnn_list, label_list1
