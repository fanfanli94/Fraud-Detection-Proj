...
parser.add_argument('--lambda_1', type=float, default=1, help='Simi loss weight.')
parser.add_argument('--lambda_2', type=float, default=1e-3, help='Weight decay (L2 loss weight).')
parser.add_argument('--emb-size', type=int, default=128, help='Node embedding size at the last layer.')
...
parser.add_argument('--step-size', type=float, default=2e-3, help='RL action step size')
...
if args.data == 'yelp':
	...
	index = list(range(4572, len(labels)))
	idx_train, idx_test, y_train, y_test = train_test_split(index, labels[4572:], stratify=labels[4572:],
...
for epoch in range(args.num_epochs):
	...
	if epoch % args.test_epochs == 0:
		if args.model == 'SAGE':
			...
			gnn_auc, label_auc, gnn_recall, label_recall, gnn_result, label_result = test_care(idx_test, y_test, gnn_model, args.batch_size)
			performance_log.append([gnn_auc, label_auc, gnn_recall, label_recall])
			x.extend(idx_test)
			y.extend(y_test)
			z.extend(gnn_result)
			w.extend(label_result)
	
	# gnn_results = [x, y, z] 
	# label_results = [x, y, w]

	# df1 = pd.DataFrame(np.array(performance_log), columns = ["GNN_AUC", "Label_AUC", 'GNN_Recall', 'Label_Recall'])
	# df1.to_csv("performance_log.csv")
	# df2 = pd.DataFrame(np.transpose(np.array(gnn_results)), columns = ['ReviewerID', 'Ground Truth', 'Predicted Label'])
	# df2.to_csv("gnn_results.csv")
	# df3 = pd.DataFrame(np.transpose(np.array(label_results)), columns = ['ReviewerID', 'Ground Truth', 'Predicted Label'])
	# df3.to_csv("label_results.csv")
